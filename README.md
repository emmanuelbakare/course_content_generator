...
- The initial provider/model catalog includes Open AI, Anthropic, Google, DeepSeek, and Groq. In **Generation settings**, select a provider and press **Use** on one enabled model. That pair is copied onto each new curriculum or lesson job, so later settings changes do not alter work already queued.
